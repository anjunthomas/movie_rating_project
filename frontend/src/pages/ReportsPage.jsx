...
const columns = [
  columnHelper.accessor("movie_title", {
    ...
    header: () => (
      <span className="flex items-center">
        <User className="mr-2" size={16}/> Movie Title
      </span>
    )
  }),
  ...
  columnHelper.accessor("num_ratings", {
    ...
    header: () => (
     <span className="flex items-center">
        <BookType className="mr-2" size={16}/> Number of Ratings
      </span>
    )
  }),
...
export default function ReportsPage(){
  const data = MOVIES.map((m) => {
    const movieRatings = RATINGS.filter((r) => r.mid === m.mid);
    const avg = movieRatings.length
      ? (movieRatings.reduce((sum, r) => sum + r.rating, 0) / movieRatings.length).toFixed(1)
      : null;
    return { movie_title: m.movie_title, num_ratings: movieRatings.length, avg_rating: avg };
  }).filter((m) => m.num_ratings > 0).sort((a, b) => b.avg_rating - a.avg_rating);
